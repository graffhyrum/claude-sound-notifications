...
import { parseTranscriptErrors, findLastUserIndex, lockfileValidAndFresh } from "./sound-hook"
...
describe("findLastUserIndex", () => {
  ...
  it("returns -1 when no user entries", () => {
    expect(findLastUserIndex([{ role: "assistant" }, { tool_result: {} }])).toBe(-1)
  })
...
describe("lockfileValidAndFresh", () => {
  ...
  it("returns false for nonexistent path", async () => {
    expect(await lockfileValidAndFresh("/tmp/no-such-file-sound-hook-xyz")).toBe(false)
  })
